<script lang="ts">
  ...
  let api = useRootContext("Option");
  let id = `headlessui-options-option-${useId()}`;
  ...
  async function updateFocus(
    ...
    if (newState !== oldState || newSelected !== oldSelected) {
      if (newState === States.Open && newSelected) {
        $api.goToOption(Focus.Specific, id);
      }
    }
    if (newState !== oldState || newActive !== oldActive) {
      if (newState === States.Open && newActive) {
  ...
  $: updateFocus($api.state, selected, active);
