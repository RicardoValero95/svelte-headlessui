<script lang="ts">
  import type {
    ...
    ForwardedEvent,
    DispatchedEvent,
  } from "./ParsedComponent";

  export let data: ParsedComponent;
  export let manualForward = false;
  const forwardedEvents = <ForwardedEvent[]>(
    data.events.filter((e) => e.type === "forwarded")
  );
  const dispatchedEvents = <DispatchedEvent[]>(
    data.events.filter((e) => e.type === "dispatched")
  );
</script>
...
and
<a
  href="https://github.com/mattjennings/vite-plugin-sveld/"
  target="_blank"
  rel="noreffer noopener">vite-plugin-sveld</a
>.
...
{#if data.rest_props}
  <div>
    severity="information" title="This component uses $$restProps."
    <br />
    All unused props will be passed through to the underlying {@html data.rest_props.name.includes(
      ...
      ? new Intl.ListFormat({ style: "short" }).format(
          data.rest_props.name
            .split(" | ")
            .map((name) => `<code>${name}</code>`)
        )
      : `<code>${data.rest_props.name}</code>`}
    {data.rest_props.type.toLowerCase()}{data.rest_props.name.includes(" | ")
      ? "s"
      : ""} as regular attributes.
    <button href="https://svelte.dev/docs#template-syntax-attributes-and-props"
      >More Info</button
    >
  </div>
{/if}
...
{:else}
  <p>
    All DOM events are forwarded to this component's respective elements by
    default. <a href="#todo">Learn More</a>
  </p>
{/if}
